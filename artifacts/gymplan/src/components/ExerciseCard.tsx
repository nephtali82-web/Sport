...
export default function ExerciseCard({ id, ex, info, week, goal, getSets, onUpdateSet, onClear, onSetGoal }: ExerciseCardProps) {
  ...
  const bumpedThisWeek = goal.bumped;
            ...
            <div className="gb-inner">
              <div className="gb-row">
                <div>
                  ...
                  {bumpedThisWeek && <div style={{ fontFamily: 'Space Mono, monospace', fontSize: '8px', color: '#4CC97A', marginTop: '2px' }}>↑ Gewicht erhöht (Double Progression)</div>}
                  {goal.auto && !bumpedThisWeek && <div style={{ fontFamily: 'Space Mono, monospace', fontSize: '8px', color: '#7A7060', marginTop: '2px' }}>Auto-berechnet aus Vorwoche</div>}
                </div>
              </div>
              ...
              {Array.from({ length: ex.sets }, (_, i) => i + 1).map(setNum => {
                ...
                return (
                  <div key={setNum} className="set-row">
                    ...
                    <input
                      type="number"
                      className={`set-input ${sd.weight ? 'filled' : ''}`}
                      placeholder={String(goal.kg || '')}
                      value={sd.weight}
                      onChange={e => onUpdateSet(setNum, 'weight', e.target.value)}
                    />
                    <input
                      type="number"
                      className={`set-input ${sd.reps ? 'filled' : ''}`}
                      placeholder={String(goal.repsTarget || ex.repsMin)}
                      value={sd.reps}
                      onChange={e => onUpdateSet(setNum, 'reps', e.target.value)}
                    />
                    <button
                      className={`sdone ${sd.done ? 'done' : ''}`}
                      onClick={() => onUpdateSet(setNum, 'done', !sd.done)}
                    >
                      ✓
                    </button>
                  </div>
